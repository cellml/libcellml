...
The libCellML library uses the :cmake:`Cmake <>` build configuration tool to configure the library.
Version 3.2 or greater of :cmake:`Cmake <>` is required to configure libCellML.
...
Please note that in :cmake:`Cmake <>` GUI Configuration applications, the config variable is prefixed with ``LIBCELLML_``
...
When configuring libCellML on `Windows <https://en.wikipedia.org/wiki/Microsoft_Windows>`_, we may need to set the location of the :libxml2:`LibXML2 <>` library, which is dependent on the computer's environment settings.
We can set the location of the :libxml2:`LibXML2 <>` library when we configure libCellML.
When we configure libCellML, the location of :libxml2:`LibXML2 <>` can be specified through the command line by adding the parameter::

  -DLibXml2_DIR="C:\Program Files\libxml2 2.9.6\lib\cmake"

to the configuration command.
This assumes that the recommended :libxml2:`LibXML2 <>` binaries have been installed to the default location ``C:\Program Files\libxml2 2.9.6``.
Please note that this method will only work with the recommended :libxml2:`LibXML2 <>` binaries, :libxml2:`LibXML2 <>` binaries from other sources will not work in this way.
...
When we press the *Configure* button, :cmake:`Cmake <>` performs an initial configuration.
This initial configuration is likely to encounter an error because :cmake:`Cmake <>` is not able to find :libxml2:`LibXML2 <>`.
...
We can resolve this error easily if we set the value of the ``LibXml2_DIR`` variable to the location of the :libxml2:`LibXML2 <>` ``cmake`` directory.
...
If :libxml2:`LibXML2 <>` was not installed to ``C:\Program Files\libxml2 2.9.6``, you will need to adjust the path to match your situation.

Build
=====

Once the build scripts have been generated by :cmake:`Cmake <>`, invoke the build with the appropriate command.
