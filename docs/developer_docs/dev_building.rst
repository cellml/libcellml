...
The variable :code:`LIBCELLML_SRC` refers to the directory containing the
:code:`LICENSE` file for libCellML.

Creating a build directory
==========================
It is best to build libCellML outside of the source tree.  To this end, create
a build directory that is not the :code:`LIBCELLML_SRC` directory.  A sibling
directory of :code:`LIBCELLML_SRC` is a good choice, named something like
:code:`build` or :code:`libcellml-build`.  We'll use the term
:code:`LIBCELLML_BUILD` to refer to this build directory.
...
The command line options can be set with the :code:`-D` flag, like so
:code:`-DBUILD_TYPE=Release`.  Please note that in :cmake:`Cmake <>` GUI
...
location :code:`C:\Program Files\libxml2 2.9.6`.  Please note that this method
will only work with the recommended LibXML2 binaries downloaded from the
...
We can resolve this error easily if we set the value of the
:code:`LibXml2_DIR:code:`
variable to the location of the :libxml2:`LibXML2 <>` :code:`cmake` directory.
:numref:`fig_devBuilding_windowsCMakeLibXml2DIRNotFound` shows the
:code:`LibXml2_DIR` variable with the value of :code:`LibXml2_DIR-NOTFOUND`.
...
.. figure:: images/libCellMLBuilding-CMakeWindowsLibXml2DIRNotFound.png
   ...
   :code:`LibXml2_DIR` variable with a value of :code:`LibXml2_DIR-NOTFOUND`.

Setting the value of :code:`LibXml2_DIR` to
:code:`C:\Program Files\libxml2 2.9.6\lib\cmake` and configuring again will
result in a successful configuration
(:numref:`fig_devBuilding_windowsCMakeLibXml2DirSet`
shows a successfully
configured :code:`LibXml2_DIR` variable) from which build files may be
generated using the *Generate* button.
...
.. figure:: images/libCellMLBuilding-CMakeWindowsLibXml2DirSet.png
   ...
   :code:`LibXml2_DIR` variable with a value of :code:`C:\Program Files\libxml2 2.9.6\lib\cmake`.

If :libxml2:`LibXML2 <>` was not installed to
:code:`C:\Program Files\libxml2 2.9.6` then you will need to adjust the path to
...
For :code:`Makefile:code:`-based configurations, the command is simply::
