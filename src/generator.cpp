...
void Generator::GeneratorImpl::processComponent(const ComponentPtr &component)
{
    ...
    for (size_t i = 0; i < component->variableCount(); ++i) {
        ...
        if (!variable->initialValue().empty()
            ...
            ModelPtr trackedVariableModel = parentModel(trackedVariableComponent);
            ErrorPtr err = std::make_shared<Error>();
...
void Generator::GeneratorImpl::processEquationAst(const GeneratorEquationAstPtr &ast)
{
    ...
    if ((ast->mType == GeneratorEquationAst::Type::CI)
        ...
        if (mVoi == nullptr) {
            ...
            ModelPtr voiModel = parentModel(voiComponent);
            ComponentPtr component = std::dynamic_pointer_cast<Component>(variable->parent());
            ModelPtr model = parentModel(component);
            ComponentPtr c1 = std::dynamic_pointer_cast<Component>(component->parent());
            ErrorPtr err = std::make_shared<Error>();

            err->setDescription("Variable '" + getEntityName(mVoi)
                                + "' in component '" + getEntityName(voiComponent)
                                + "' of model '" + getEntityName(voiModel)
                                + "' and variable '" + getEntityName(variable)
                                + "' in component '" + getEntityName(component)
                                + "' of model '" + getEntityName(model)
    ...
    if ((ast->mType == GeneratorEquationAst::Type::CN)
        ...
        if (convertToDouble(ast->mValue) != 1.0) {
            ...
            err->setDescription("The differential equation for variable '" + getEntityName(variable)
                                + "' in component '" + getEntityName(component)
                                + "' of model '" + getEntityName(model)
    ...
    if (mGenerator->errorCount() == 0) {
        for (const auto &internalVariable : mInternalVariables) {
            ...
            if (!errorType.empty()) {
                ...
                ModelPtr realModel = parentModel(realComponent);
