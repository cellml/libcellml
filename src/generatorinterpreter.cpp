                                                                ...
                                                                const std::string &code)
{
    ...
    // Add code for the implementation to compute our rates (and any variables on which they depend).
                                                                                 ...
                                                                                 const AnalyserEquationAstPtr &ast) const
{
    ...
    auto leftCode = generateCode(astLeftChild);
    auto rightCode = generateCode(astRightChild);
...
std::string GeneratorInterpreter::GeneratorInterpreterImpl::generateTwoParameterFunctionCode(const std::string &function,
                                                                                             const AnalyserEquationAstPtr &ast) const
{
    auto leftCode = generateCode(ast->leftChild());
    auto rightCode = generateCode(ast->rightChild());
...
std::string GeneratorInterpreter::GeneratorInterpreterImpl::generateCode(const AnalyserEquationAstPtr &ast) const
{
    ...
    case AnalyserEquationAst::Type::MINUS:
        code = (ast->rightChild() != nullptr) ?
                   generateOperatorCode(mProfile->minusString(), ast) :
                   generateMinusUnaryCode(ast);
    ...
    case AnalyserEquationAst::Type::POWER: {
        ...
        if (validConversion && areEqual(doubleValue, 0.5)) {
            code = mProfile->squareRootString() + "(" + generateCode(ast->leftChild()) + ")";
        } else if (validConversion && areEqual(doubleValue, 2.0)
                   && !mProfile->squareString().empty()) {
            code = mProfile->squareString() + "(" + generateCode(ast->leftChild()) + ")";
        } else if (mProfile->hasPowerOperator()) {
            code = generateOperatorCode(mProfile->powerString(), ast);
        } else {
            code = mProfile->powerString() + "(" + generateCode(ast->leftChild()) + ", " + rightCode + ")";
        }
    } break;
        ...
        if (astRightChild != nullptr) {
            ...
            if (validConversion && areEqual(doubleValue, 2.0)) {
                code = mProfile->squareRootString() + "(" + generateCode(astRightChild) + ")";
                       ...
                       && !mProfile->squareString().empty()) {
                code = mProfile->squareString() + "(" + generateCode(astRightChild) + ")";
            } else if (mProfile->hasPowerOperator()) {
                code = generateOperatorCode(mProfile->powerString(), ast);
            } else {
                ...
                code = mProfile->powerString() + "(" + generateCode(astRightChild) + ", " + generateOperatorCode(mProfile->divideString(), inverseValueAst) + ")";
            }
        } else {
            code = mProfile->squareRootString() + "(" + generateCode(ast->leftChild()) + ")";
        }
