...
bool checkUnitsForCycles(const ModelPtr &origModel, const UnitsPtr &units, HistorySearchVector &history, IssueList &issueList)
{
    ...
    if (!units->isImport()) {
        for (size_t u = 0; u < units->unitCount(); ++u) {
            ...
            if ((model != nullptr) && model->hasUnits(ref)) {
                if (checkUnitsForCycles(origModel, model->units(ref), history, issueList)) {
                    return true;
                }
            }
        }
    ...
    if (std::find(history.begin(), history.end(), h) == history.end()) {
        ...
        return checkUnitsForCycles(origModel, importedUnits, history, issueList);
    }
    // Otherwise, return true indicating that cycles have been found.
    history.emplace_back(h);
    issueList.emplace_back(makeIssueCyclicDependency(origModel, "units", history, "flatten"));
    return true;
}

bool checkComponentForCycles(const ModelPtr &origModel, const ComponentPtr &component, HistorySearchVector &history, IssueList &issueList)
{
    ...
    if (std::find(history.begin(), history.end(), h) == history.end()) {
        ...
        if (component->isImport()) {
            ...
            if (importedComponent->isImport() && checkComponentForCycles(origModel, importedComponent, history, issueList)) {
                return true;
            }
        }
        return false;
    ...
    // Otherwise, return true indicating that cycles have been found.
    history.emplace_back(h);
    issueList.emplace_back(makeIssueCyclicDependency(origModel, "component", history, "flatten"));
    return true;
}
...
bool checkModelForCycles(const ModelPtr &model, IssueList &issueList)
{
    ...
    for (size_t i = 0; i < model->importSourceCount(); ++i) {
        ...
        for (size_t u = 0; u < importSource->unitsCount(); ++u) {
            if (checkUnitsForCycles(model, importSource->units(u), history, issueList)) {
                return true;
            }
        }
        for (size_t c = 0; c < importSource->componentCount(); ++c) {
            if (checkComponentForCycles(model, importSource->component(c), history, issueList)) {
                return true;
            }
        }
    }
