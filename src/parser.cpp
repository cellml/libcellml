...
void Parser::ParserImpl::loadModel(const ModelPtr &model, const std::string &input)
{
    ...
    if (!node->isCellmlElement("model")) {
        ...
        err->setModel(model);
        err->setRule(ReferenceRule::MODEL_ELEMENT);
        mParser->addIssue(err);
        return;
    }
    ...
    while (childNode) {
        if (childNode->isCellmlElement("component")) {
            ...
            if (componentRefNode) {
                ...
                err->setModel(model);
                err->setCause(Issue::Cause::ENCAPSULATION);
                err->setRule(ReferenceRule::ENCAPSULATION_COMPONENT_REF);
                mParser->addIssue(err);
            }
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                ...
                err->setModel(model);
                err->setRule(ReferenceRule::MODEL_CHILD);
                mParser->addIssue(err);
            }
    ...
    if (!encapsulationNodes.empty()) {
        ...
        if (encapsulationNodes.size() > 1) {
            ...
            err->setModel(model);
            err->setCause(Issue::Cause::ENCAPSULATION);
            err->setRule(ReferenceRule::MODEL_MORE_THAN_ONE_ENCAPSULATION);
            mParser->addIssue(err);
        }
    }
    ...
    while (childNode) {
        if (childNode->isCellmlElement("variable")) {
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                ...
                err->setComponent(component);
                err->setRule(ReferenceRule::COMPONENT_CHILD);
                mParser->addIssue(err);
            }
        } else if (childNode->isComment()) {
            // Do nothing.
    ...
    while (attribute) {
        if (attribute->isType("units")) {
            ...
            if (isCellMLReal(attribute->value())) {
                ...
                err->setUnits(units);
                err->setRule(ReferenceRule::UNIT_EXPONENT);
                mParser->addIssue(err);
            }
            ...
            if (isCellMLReal(attribute->value())) {
                ...
                err->setUnits(units);
                err->setRule(ReferenceRule::UNIT_MULTIPLIER);
                mParser->addIssue(err);
            }
        } else if (attribute->isType("id")) {
            id = attribute->value();
            ...
            err->setUnits(units);
            err->setRule(ReferenceRule::UNIT_OPTIONAL_ATTRIBUTE);
            mParser->addIssue(err);
        }
        attribute = attribute->next();
    }
...
void Parser::ParserImpl::loadConnection(const ModelPtr &model, const XmlNodePtr &node)
{
    ...
    if (component1Name.empty()) {
        ...
        err->setModel(model);
        err->setCause(Issue::Cause::CONNECTION);
        err->setRule(ReferenceRule::CONNECTION_COMPONENT1);
        mParser->addIssue(err);
    ...
    if (component2Name.empty()) {
        ...
        err->setModel(model);
        err->setCause(Issue::Cause::CONNECTION);
        err->setRule(ReferenceRule::CONNECTION_COMPONENT2);
        mParser->addIssue(err);
    ...
    while (childNode) {
        ...
        if (childNode->isCellmlElement("map_variables")) {
            ...
            if (variable1Name.empty()) {
                ...
                err->setModel(model);
                err->setCause(Issue::Cause::CONNECTION);
                err->setRule(ReferenceRule::MAP_VARIABLES_VARIABLE1);
                mParser->addIssue(err);
            ...
            if (variable2Name.empty()) {
                ...
                err->setModel(model);
                err->setCause(Issue::Cause::CONNECTION);
                err->setRule(ReferenceRule::MAP_VARIABLES_VARIABLE2);
                mParser->addIssue(err);
    ...
    while (attribute) {
        if (attribute->isType("component")) {
            ...
            if (model->containsComponent(parentComponentName)) {
                ...
                err->setModel(model);
                err->setCause(Issue::Cause::ENCAPSULATION);
                err->setRule(ReferenceRule::COMPONENT_REF_COMPONENT_ATTRIBUTE);
                mParser->addIssue(err);
            }
        } else if (attribute->isType("id")) {
    ...
    while (componentRefNode) {
        ...
        if (componentRefNode->isCellmlElement("component_ref")) {
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                ...
                err->setModel(model);
                err->setCause(Issue::Cause::ENCAPSULATION);
                err->setRule(ReferenceRule::COMPONENT_REF_CHILD);
                mParser->addIssue(err);
            } else {
    ...
    while (childNode) {
        if (childNode->isCellmlElement("component")) {
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                ...
                err->setImportSource(importSource);
                err->setRule(ReferenceRule::IMPORT_CHILD);
                mParser->addIssue(err);
            }
        } else if (childNode->isComment()) {
            // Do nothing.
...
void Parser::ParserImpl::loadResetChild(const std::string &childType, const ResetPtr &reset, const ComponentPtr &component, const XmlNodePtr &node)
{
    ...
    while (childAttribute) {
        if (childAttribute->isType("id")) {
            ...
            IssuePtr err = Issue::create();
            err->setDescription("Reset in component '" + component->name() + "' referencing variable '" + variableName + "' and test_variable '" + testVariableName + "' has an unexpected attribute in the " + childType + " block of '" + childAttribute->name() + "'.");
            err->setReset(reset);
    ...
    while (mathNode) {
        if (mathNode->isMathmlElement("math")) {
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                IssuePtr err = Issue::create();
                err->setDescription("The " + childType + " in the reset in component '" + component->name() + "' referencing variable '" + variableName + "' and test_variable '" + testVariableName + "' should have a MathML block as a child.");
                err->setReset(reset);
                err->setRule(ReferenceRule::RESET_TEST_VALUE);
                mParser->addIssue(err);
            }
...
void Parser::ParserImpl::loadReset(const ResetPtr &reset, const ComponentPtr &component, const XmlNodePtr &node)
{
    ...
    while (childNode) {
        if (childNode->isCellmlElement("test_value")) {
            ...
            if (hasNonWhitespaceCharacters(textNode)) {
                IssuePtr err = Issue::create();
                err->setDescription("Reset has an invalid non-whitespace child text element '" + textNode + "'. Either a test_value block or a reset_value block is expected.");
                err->setReset(reset);
                err->setRule(ReferenceRule::RESET_CHILD);
                mParser->addIssue(err);
            }
        } else if (childNode->isComment()) {
            // Do nothing.
