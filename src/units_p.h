...
/**
 * @brief The UnitsPrivate class.
 *
 * The private implementation for the Units class.
 */
class UnitsPrivate : public NamedEntityPrivate, public ImportedEntityPrivate
{
public:
...
std::vector<UnitDefinition>::iterator UnitsPrivate::findUnit(const std::string &reference)
{
    return std::find_if(mUnits.begin(), mUnits.end(),
                        [=](const UnitDefinition &u) -> bool { return u.mReference == reference; });
}

bool UnitsPrivate::isBaseUnit(const std::string &name) const
{
