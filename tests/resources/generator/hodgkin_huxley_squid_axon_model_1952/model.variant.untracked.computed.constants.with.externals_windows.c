...
#include "model.variant.untracked.computed.constants.with.externals.h"
...
{
    computedConstants[0] = 12.0+constants[1];
}
...
void computeRates(double voi, double *states, double *rates, double *constants, double *computedConstants, double *algebraicVariables, double *externalVariables, ExternalVariable externalVariable)
{
    ...
    double leakage_current_E_L = -10.613+constants[1];
    algebraicVariables[1] = states[0]*constants[2]-constants[2]*leakage_current_E_L;
    algebraicVariables[2] = states[0]*pow(states[3], 4.0)*constants[4]-pow(states[3], 4.0)*constants[4]*computedConstants[0];
