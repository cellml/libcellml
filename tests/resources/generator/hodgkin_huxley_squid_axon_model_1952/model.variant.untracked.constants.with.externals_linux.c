...
#include "model.variant.untracked.constants.with.externals.h"
...
const size_t ALGEBRAIC_VARIABLE_COUNT = 9;
...
void computeComputedConstants(double voi, double *states, double *rates, double *constants, double *computedConstants, double *algebraicVariables)
{
    ...
    computedConstants[0] = -10.613+membrane_E_R;
    computedConstants[1] = -115.0+membrane_E_R;
    computedConstants[2] = 12.0+membrane_E_R;
}
...
void computeRates(double voi, double *states, double *rates, double *constants, double *computedConstants, double *algebraicVariables, double *externalVariables, ExternalVariable externalVariable)
{
    ...
    algebraicVariables[2] = -pow(states[3], 4.0)*potassium_channel_g_K*computedConstants[2]+states[0]*pow(states[3], 4.0)*potassium_channel_g_K;
    double leakage_current_g_L = 0.3;
    algebraicVariables[1] = states[0]*leakage_current_g_L-leakage_current_g_L*computedConstants[0];
    algebraicVariables[3] = 0.1*states[0]/(-1.0+exp(2.5+0.1*states[0]))+2.5/(-1.0+exp(2.5+0.1*states[0]));
    ...
    rates[0] = (algebraicVariables[0]-externalVariables[0]-algebraicVariables[1]-algebraicVariables[2])/constants[0];
    algebraicVariables[4] = 4.0*exp(0.0555555555555556*states[0]);
    rates[2] = -states[2]*algebraicVariables[4]+(1.0-states[2])*algebraicVariables[3];
    algebraicVariables[5] = 0.07*exp(0.05*states[0]);
    algebraicVariables[6] = 1.0/(1.0+exp(3.0+0.1*states[0]));
    rates[1] = -states[1]*algebraicVariables[6]+(1.0-states[1])*algebraicVariables[5];
    algebraicVariables[7] = 0.01*states[0]/(-1.0+exp(1.0+0.1*states[0]))+0.1/(-1.0+exp(1.0+0.1*states[0]));
    algebraicVariables[8] = 0.125*exp(0.0125*states[0]);
    rates[3] = -states[3]*algebraicVariables[8]+(1.0-states[3])*algebraicVariables[7];
}
