...
#include "model.variant.untracked.computed.constants.h"
...
{
    double sodium_channel_E_Na = -115.0+constants[1];
    algebraicVariables[3] = states[0]*states[1]*pow(states[2], 3.0)*constants[3]-states[1]*pow(states[2], 3.0)*constants[3]*sodium_channel_E_Na;
    algebraicVariables[0] = ((voi >= 10.0) && (voi <= 10.5))?-20.0:0.0;
    double leakage_current_E_L = -10.613+constants[1];
    algebraicVariables[1] = states[0]*constants[2]-leakage_current_E_L*constants[2];
    double potassium_channel_E_K = 12.0+constants[1];
    algebraicVariables[2] = -pow(states[3], 4.0)*potassium_channel_E_K*constants[4]+states[0]*pow(states[3], 4.0)*constants[4];
