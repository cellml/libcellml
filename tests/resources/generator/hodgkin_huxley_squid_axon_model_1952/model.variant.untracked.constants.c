...
#include "model.variant.untracked.constants.h"
...
void computeRates(double voi, double *states, double *rates, double *constants, double *computedConstants, double *algebraicVariables)
{
    ...
    algebraicVariables[3] = states[0]*states[1]*pow(states[2], 3.0)*sodium_channel_g_Na-states[1]*pow(states[2], 3.0)*sodium_channel_g_Na*computedConstants[1];
    algebraicVariables[0] = ((voi >= 10.0) && (voi <= 10.5))?-20.0:0.0;
    double leakage_current_g_L = 0.3;
    algebraicVariables[1] = states[0]*leakage_current_g_L-computedConstants[0]*leakage_current_g_L;
    double potassium_channel_g_K = 36.0;
    algebraicVariables[2] = -pow(states[3], 4.0)*computedConstants[2]*potassium_channel_g_K+states[0]*pow(states[3], 4.0)*potassium_channel_g_K;
    rates[0] = (-algebraicVariables[2]-algebraicVariables[1]+algebraicVariables[0]-algebraicVariables[3])/membrane_Cm;
