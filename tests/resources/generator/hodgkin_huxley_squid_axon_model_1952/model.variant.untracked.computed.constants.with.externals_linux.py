...
def compute_computed_constants(voi, states, rates, constants, computed_constants, algebraic_variables):
    computed_constants[0] = 12.0+constants[1]


def compute_rates(voi, states, rates, constants, computed_constants, algebraic_variables, external_variables, external_variable):
    algebraic_variables[2] = -pow(states[3], 4.0)*constants[4]*computed_constants[0]+states[0]*pow(states[3], 4.0)*constants[4]
    leakage_current_E_L = -10.613+constants[1]
    algebraic_variables[1] = states[0]*constants[2]-constants[2]*leakage_current_E_L
    algebraic_variables[3] = 0.1*states[0]/(-1.0+exp(2.5+0.1*states[0]))+2.5/(-1.0+exp(2.5+0.1*states[0]))
    ...
    rates[0] = (algebraic_variables[0]-external_variables[0]-algebraic_variables[1]-algebraic_variables[2])/constants[0]
    algebraic_variables[4] = 4.0*exp(0.0555555555555556*states[0])
    rates[2] = -states[2]*algebraic_variables[4]+(1.0-states[2])*algebraic_variables[3]
    algebraic_variables[5] = 0.07*exp(0.05*states[0])
    algebraic_variables[6] = 1.0/(1.0+exp(3.0+0.1*states[0]))
    rates[1] = -states[1]*algebraic_variables[6]+(1.0-states[1])*algebraic_variables[5]
    algebraic_variables[7] = 0.01*states[0]/(-1.0+exp(1.0+0.1*states[0]))+0.1/(-1.0+exp(1.0+0.1*states[0]))
    algebraic_variables[8] = 0.125*exp(0.0125*states[0])
    rates[3] = -states[3]*algebraic_variables[8]+(1.0-states[3])*algebraic_variables[7]
