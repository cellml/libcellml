...
#include "model.variant.untracked.algebraic.variables.h"
...
void computeRates(double voi, double *states, double *rates, double *constants, double *computedConstants, double *algebraicVariables)
{
    ...
    double leakage_current_i_L = states[0]*constants[2]-computedConstants[0]*constants[2];
    double potassium_channel_i_K = -pow(states[3], 4.0)*computedConstants[2]*constants[4]+states[0]*pow(states[3], 4.0)*constants[4];
    double sodium_channel_i_Na = -states[1]*pow(states[2], 3.0)*constants[3]*computedConstants[1]+states[0]*states[1]*pow(states[2], 3.0)*constants[3];
    rates[0] = (-sodium_channel_i_Na-potassium_channel_i_K-leakage_current_i_L+membrane_i_Stim)/constants[0];
    ...
    rates[2] = (1.0-states[2])*sodium_channel_m_gate_alpha_m-states[2]*sodium_channel_m_gate_beta_m;
