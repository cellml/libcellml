...
def compute_rates(voi, states, rates, constants, computed_constants, algebraic_variables, external_variables, external_variable):
    ...
    potassium_channel_i_K = -pow(states[3], 4.0)*potassium_channel_g_K*computed_constants[0]+states[0]*pow(states[3], 4.0)*potassium_channel_g_K
    ...
    rates[0] = (membrane_i_Stim-leakage_current_i_L-external_variables[0]-potassium_channel_i_K)/constants[0]
    sodium_channel_m_gate_beta_m = 4.0*exp(0.0555555555555556*states[0])
    rates[2] = -states[2]*sodium_channel_m_gate_beta_m+(1.0-states[2])*algebraic_variables[0]
