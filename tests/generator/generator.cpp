...
TEST(Generator, hodgkinHuxleySquidAxonModel1952Imports)
{
    libcellml::Parser parser;
    libcellml::ModelPtr model = parser.parseModel(fileContents("generator/imports/hodgkin_huxley_squid_axon_model_1952/model.cellml"));

    EXPECT_EQ(size_t(0), parser.errorCount());

    EXPECT_TRUE(model->hasUnresolvedImports());
    model->resolveImports(resourcePath("generator/imports/hodgkin_huxley_squid_axon_model_1952/"));
